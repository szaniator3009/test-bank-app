...
import { AccountEventTransaction, TransferBoxFormService } from '../transactions/transfer-box-form/services/transferBoxFormService.service';
...
export class ModalComponent implements OnInit, OnDestroy {
  ...
  private addAccountEvent$(): Observable<any> {
    return this.transferBoxFormService._accountEvent$.pipe(
      switchMap((event) => {
        this.transactionsService.postAccountEvent$(this.transferBoxFormService.createAccountEvent(event));
        return of(event);
      })
    );
  }
